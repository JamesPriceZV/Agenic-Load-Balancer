...
- UI validation now has launch coverage, minimum-window flow coverage, deterministic workspace/task coverage, the Sprint L smoke matrix, the Sprint N expanded app-surface matrix, and the Sprint O light/dark/maximized/provider-edge/run-sheet matrix. Remaining visual coverage should focus on Q.2 remediation flows: Conflict Center recovery, continuation panel depth states, provider badge-strip remediation actions, Foundation Models diagnostics variants, full-screen permutations, and baseline-history comparison.
...
- Provider setup UX: Sprint P.2 landed the four-badge strip (binary / auth / credentials / freshness) plus the tone roll-up; remaining future work is per-provider docs snapshots, in-app remediation buttons that run the remediation command behind an approval prompt, and trendline freshness gating.
- Visual regression coverage expansion: build out Sprint Q.2 matrices for the Conflict Center recovery flow, run-sheet continuation depth, provider badge-strip remediation actions, and Foundation Models diagnostics surface so they share the same baselines as the existing Sprint L/N/O matrices.
...
The remaining queue is now physical two-machine CloudKit conflict recovery execution on Solaris551 and Solaris971 with peer evidence attached, Q.2 screenshot-diff/remediation polish for conflict recovery, continuation panel depth, provider badge remediation, Foundation Models diagnostics, and full-screen permutations, ongoing provider probe upkeep as CLIs drift, periodic live Foundation Models checks on eligible macOS 26.x machines, and notarization/stapling after a `notarytool` keychain profile is selected. Fully autonomous continuation loops remain bounded and approval-gated by design; Sprint M through Sprint O verify those safety seams instead of removing them.
